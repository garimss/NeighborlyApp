...
def add_ad_request():
    ...
    wurl = urljoin(settings.API_URL + '/getAdvertisement?id=' + id)
...
def update_ad_request(id):
    ...
    wurl= urljoin(settings.API_URL + '/updateAdvertisement?id=' + id)
    response = requests.put(wurl, json.dumps(req_data))
    return redirect(url_for('home'))

@app.route('/ad/delete/<id>', methods=['POST'])
def delete_ad_request(id):
    wurl= urljoin(settings.API_URL + '/deleteAdvertisement?id=' + id)
